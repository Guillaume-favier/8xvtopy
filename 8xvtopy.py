...
if len(sys.argv) != 1 and sys.argv[1] == '-h':
    print(
        '8xv to py is simple program in python to convert python appvar from ti83 Premium CE python Edition to python files\nUsage : python3 pyto8xv.py <file1> <file2> <file3> ...\n You can Activate the verbose mod by doing \'-v\''
    )
    exit()
...
def conv(inp, name='', meth=0):
    ...
    try:
        ii = i3.split(bytes(
            '\x2E\x70\x79\x00',
            'utf-8'))  ### try to find the .py of the convertion of the file
        iii = ii[1]
        #print(str(ii[0]))
        name = ii[0].split(bytes('\x01', 'utf-8'))[1].decode('utf-8')
        verbose('The file was originally created on a pc with the name : ' +
                name + ".py")
        ### TEST OF THE SECOND METHOD
    except IndexError:
        tmp = i2
        ii = tmp.split(
            bytes('\x50\x59\x43\x44\x00', 'utf-8')
        )  ### try to find the PYCD.p to separate the header with the name of the file and the content
        iii = ii[0].split(bytes('\x15',
                                'utf-8'))  ### get the name of the file in
        #print(iii)
        name = iii[1].split(bytes('\x00\x00', 'utf-8'))[0].decode('utf-8')
        #print(name)
        #print(str(ii[1].decode('utf-8')))
        verbose(
            'The file was originally created on a calculator with the name : '
            + name)
        pass
    ...
    try:
        ...
        verbose('Removing the two last caracter : ' +
                str(pcont[len(pcont) - 2:]))
    except UnicodeDecodeError:
        print(
            'File corupt. Please remove weird caracters at the end of the file but let the 2 first'
        )
    #print(cont)
    name = str(name)
...
### if there is multiples files
    ...
    for x in range(len(sys.argv)):
        if x == 0:
            ...
            print('Start of : ' + sys.argv[x] + ' ' + str(x) + '/' +
                  str(len(sys.argv) - 2))
            conv(sys.argv[x])
        pass
    print('All done!')
